...
class UsbTest(Screen):
	...
	def __init__(self,session,testitem):
		...
		self.devpath = "sda/sda1"
		self.usbdevice = Harddisk(self.devpath)
#		self.usbdevicename = self.usbdevice.model()
		self.usbdevicename = readFile("/sys/block/sda/device/vendor")
...
#		self.usbfreesize = self.usbdevice.free()
		...
		self["usbtitle"] = StaticText("USB Device:"+self.usbdevicename)
		self["usbname"] = StaticText("USB Name: "+self.usbdevicename)
		self["usbsize"] = StaticText("USB Size: "+self.usbsize)
